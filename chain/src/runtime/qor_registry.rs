//! QorID Registry module for user profiles and Syzygy tracking.
//!
//! This module handles:
//! - QorID profiles (display name, bio, Syzygy Score)
...
const PREFIX_QORID_PROFILE: &[u8] = b"qorid/profile:";
const PREFIX_QORID_HANDLE: &[u8] = b"qorid/handle/";
const PREFIX_USERNAME: &[u8] = b"username/";

// Badge threshold
const LUMINARY_SYZYGY_THRESHOLD: u64 = 10_000;

/// QorID profile with Syzygy tracking.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QorIDProfile {
    pub address: Address,
...
/// QorID profile management

fn abyssid_profile_key(address: &Address) -> Vec<u8> {
    let mut key = Vec::with_capacity(PREFIX_QORID_PROFILE.len() + address.len());
    key.extend_from_slice(PREFIX_QORID_PROFILE);
    key.extend_from_slice(address);
    key
}

fn load_qorid_profile(state: &State, address: &Address) -> Option<QorIDProfile> {
    state
        .get_raw(&abyssid_profile_key(address))
        .and_then(|bytes| bincode::deserialize::<QorIDProfile>(&bytes).ok())
}

fn store_qorid_profile(state: &mut State, profile: &QorIDProfile) -> Result<(), String> {
...
fn handle_key(handle: &str) -> Vec<u8> {
    let mut key = Vec::with_capacity(PREFIX_QORID_HANDLE.len() + handle.len());
    key.extend_from_slice(PREFIX_QORID_HANDLE);
...
pub fn set_username(
    ...
    let mut profile = load_qorid_profile(state, caller)
        .ok_or_else(|| "QorID profile not found".to_string())?;
    ...
    store_qorid_profile(state, &profile)?;
...
pub fn create_qorid_profile(
    state: &mut State,
    address: Address,
    display_name: String,
    bio: Option<String>,
    current_height: u64,
) -> Result<QorIDProfile, String> {
    if load_qorid_profile(state, &address).is_some() {
        return Err("QorID profile already exists for this address".into());
    }

    let profile = QorIDProfile {
        address,
        display_name,
        bio,
...
/// Set or update an QorID's handle.
///
...
pub fn set_handle(
    ...
    let mut profile = load_qorid_profile(state, &address)
        .ok_or_else(|| "QorID profile not found".to_string())?;
...
/// Get an QorID profile by address.
pub fn get_qorid_profile(state: &State, address: &Address) -> Option<QorIDProfile> {
    load_qorid_profile(state, address)
}
...
/// QorIDRegistryModule handles QorID profiles and Syzygy tracking
pub struct QorIDRegistryModule;

impl QorIDRegistryModule {
    pub fn new() -> Self {
        Self
    }
}

impl RuntimeModule for QorIDRegistryModule {
    ...
    fn dispatch(&self, call_id: &str, tx: &Transaction, state: &mut State) -> Result<(), String> {
        match call_id {
            ...
            other => Err(format!("qor_registry: unknown call_id '{}'", other)),
        }
    }
}
...
mod tests {
    ...
    fn test_claim_archon() {
        ...
        let tx = Transaction {
            from: addr,
            nonce: 0,
            module_id: "qor_registry".to_string(),
        ...
        let module = QorIDRegistryModule::new();
    ...
    #[test]
    fn test_create_qorid_profile() {
        let mut state = State::in_memory();
        let addr = [1u8; 32];

        let profile = create_qorid_profile(
            &mut state,
            addr,
            "Test QorID".to_string(),
            Some("Test bio".to_string()),
            0,
        )
        .unwrap();

        assert_eq!(profile.display_name, "Test QorID");
        assert_eq!(profile.syzygy_score, 0);
        assert_eq!(profile.badges, Vec::<String>::new());

        // Should fail on duplicate
        assert!(create_qorid_profile(
            &mut state,
            addr,
            "Another".to_string(),
            None,
    ...
    fn test_record_syzygy() {
        ...
        create_qorid_profile(&mut state, addr, "Test".to_string(), None, 0).unwrap();
