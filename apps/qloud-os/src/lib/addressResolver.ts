 ...
 * Address Resolver
 * 
 * Resolves blockchain addresses to QorID usernames with caching.
...
/**
 * Resolve an address to an QorID profile
 */
async function fetchProfile(address: string): Promise<QorIDProfile | null> {
  try {
    ...
    if (json.result && json.result.exists !== false) {
      return json.result as QorIDProfile;
    }
    return null;
  } catch (error) {
    console.error('Failed to fetch QorID profile:', error);
    return null;
  }
}
