 ...
 * QorID SDK
 * 
 * High-level API for QorID identity management.
 * Provides a clean interface for creating, loading, and managing QorID sessions.
 * 
...
/**
 * Load existing QorID session from storage
 */
export function loadSession(): AbyssSession | null {
  ...
  try {
    ...
    console.warn('Failed to load QorID session:', error);
  }
...
export async function signMessage(
  ...
  if (!session) {
    throw new Error('No active QorID session');
  }
...
/**
 * QorID SDK with DRC-369 extensions
 */
