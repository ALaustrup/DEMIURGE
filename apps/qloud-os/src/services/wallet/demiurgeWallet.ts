...
export interface SendCgtParams {
  from: string;      // QorID identity public key
...
/**
 * Derive Demiurge public key from QorID identity key
 */
...
export async function canSendCgt(sessionToken: string | null): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_QORID_API_URL || 'http://localhost:8082'}/api/wallet/can-send`, {
      headers: {
...
export async function syncNftMintStatus(sessionToken: string | null): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_QORID_API_URL || 'http://localhost:8082'}/api/wallet/sync-nft-status`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${sessionToken}`,
