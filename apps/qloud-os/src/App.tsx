...
import { QorIDProvider, useQorID } from './context/QorIDContext';
...
import { migrateOldDemiNFTData } from './services/qorid/drc369';
...
function AppContent() {
  ...
  const { session, isLoading: qorIDLoading } = useQorID();
  const { initialize } = useAuthStore();
  const hasInitialized = useRef(false);
  
  // Sync authStore with QorID session
  useEffect(() => {
    if (session && !hasInitialized.current) {
      // Update authStore to reflect QorID session
      const account = {
      ...
      // Clear authStore if QorID session is lost
