...
export interface WalletState {
  // Derived Demiurge public key (from QorID identity)
  ...
  persist(
    (set, get) => ({
      ...
      refreshBalance: async () => {
        ...
        try {
          // First try to get from QorID service (includes 5000 CGT gift)
          ...
          if (sessionToken) {
            try {
              const balanceResponse = await fetch(
                `${import.meta.env.VITE_QORID_API_URL || 'http://localhost:8082'}/api/qorid/wallet/balance`,
                {
                  headers: {
