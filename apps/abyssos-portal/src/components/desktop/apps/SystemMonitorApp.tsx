...
import { getAllVMJobs, onVMJobUpdate } from '../../../services/abyssvm/abyssvm';
import { Button } from '../../shared/Button';
import type { VMJobStatus } from '../../../services/abyssvm/types';
...
export function SystemMonitorApp() {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 p-6 space-y-6 overflow-auto">
      <div>
        <h2 className="text-2xl font-bold text-abyss-cyan mb-2">System Monitor</h2>
        <p className="text-sm text-gray-400">Process management and resource usage</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-abyss-navy/50 border border-abyss-cyan/20 rounded-lg p-4">
          ...
          <div className="text-2xl font-mono text-abyss-cyan">{totalCpu.toFixed(1)}%</div>
          <div className="mt-2 h-2 bg-abyss-dark rounded-full overflow-hidden">
            <div
        ...
        <div className="bg-abyss-navy/50 border border-abyss-cyan/20 rounded-lg p-4">
          <div className="text-sm text-gray-400 mb-1">Total Memory Usage</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-abyss-cyan">Running Processes</h3>
        ...
        {processes.length === 0 ? (
          ...
          <div className="bg-abyss-dark/50 border border-abyss-cyan/20 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-abyss-navy/50 border-b border-abyss-cyan/20">
                <tr>
                  <th className="text-left p-3 text-gray-400 font-medium">PID</th>
                  ...
                  <th className="text-right p-3 text-gray-400 font-medium">CPU</th>
                  <th className="text-right p-3 text-gray-400 font-medium">Memory</th>
              ...
              <tbody>
                {processes.map((process) => (
                  <tr
                    key={process.pid}
                    className="border-b border-abyss-cyan/10 hover:bg-abyss-navy/30 transition-colors"
                  >
                    <td className="p-3 font-mono text-abyss-cyan">{process.pid}</td>
                    <td className="p-3 text-white">{process.name}</td>
                    <td className="p-3 text-gray-400">{process.type}</td>
      ...
      {gridJobs.length > 0 && (
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-abyss-cyan">Grid Jobs</h3>
            <div className="text-xs text-gray-400">
          ...
          <div className="bg-abyss-dark/50 border border-abyss-cyan/20 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-abyss-navy/50 border-b border-abyss-cyan/20">
                <tr>
                  <th className="text-left p-3 text-gray-400 font-medium">Job ID</th>
                  <th className="text-left p-3 text-gray-400 font-medium">Status</th>
                  <th className="text-left p-3 text-gray-400 font-medium">Peer</th>
              ...
              <tbody>
                {gridJobs.map((job) => (
                  <tr
                    ...
                    className="border-b border-abyss-cyan/10 hover:bg-abyss-navy/30 transition-colors"
                  >
                    <td className="p-3 font-mono text-abyss-cyan text-xs">
                    ...
                    <td className="p-3">
                      {job.progress !== undefined ? (
                        <div className="flex items-center space-x-2">
                          <div className="flex-1 h-2 bg-abyss-dark rounded-full overflow-hidden">
                            <div
                              className="h-full bg-abyss-cyan transition-all duration-300"
