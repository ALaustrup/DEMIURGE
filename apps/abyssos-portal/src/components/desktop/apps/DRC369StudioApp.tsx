...
export function DRC369StudioApp() {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 p-6 space-y-6 overflow-auto">
      <div>
        <h2 className="text-2xl font-bold text-abyss-cyan mb-2">DRC-369 Studio</h2>
        <p className="text-sm text-gray-400">Create and preview DRC-369 assets</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">Royalties (%)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">URI (IPFS/HTTP)</label>
            <input
              type="text"
              value={formData.uri}
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">Additional Metadata (JSON)</label>
            <textarea
              value={formData.metadata}
              onChange={(e) => setFormData(prev => ({ ...prev, metadata: e.target.value }))}
              placeholder='{"key": "value"}'
              rows={3}
              className="w-full px-3 py-2 bg-abyss-dark border border-abyss-cyan/30 rounded text-white text-sm font-mono focus:outline-none focus:border-abyss-cyan"
            />
          </div>
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-bold text-abyss-cyan mb-2">Preview</h3>
            <div className="bg-abyss-dark/50 border border-abyss-cyan/20 rounded-lg p-4 min-h-[400px] flex items-center justify-center">
              {preview ? (
          ...
          {formData.name && (
            <div className="bg-abyss-navy/50 border border-abyss-cyan/20 rounded-lg p-4">
              <h4 className="font-medium text-abyss-cyan mb-2">{formData.name}</h4>
