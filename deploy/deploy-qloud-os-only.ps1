...
Write-Host "🚀 Deploying QOR OS to Production Server" -ForegroundColor Green
...
# Build QOR OS
Write-Host "🔨 Building QOR OS..." -ForegroundColor Cyan
ssh $Server 'cd /opt/demiurge/repo/apps/qloud-os && sudo chown -R demiurge:demiurge . && sudo -u demiurge pnpm install && sudo -u demiurge pnpm build' 2>&1 | Write-Host
if ($LASTEXITCODE -ne 0) {
...
sudo cp -r /opt/demiurge/repo/apps/qloud-os/dist/* /opt/demiurge/web/abyssos/
