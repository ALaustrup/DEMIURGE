# Demiurge Blockchain Documentation

Welcome to the Demiurge L1 documentation. This repo contains the core chain, runtime modules, CGT token logic, work-claim mining, infrastructure for node operators, and the AbyssOS desktop environment.

**Live Services:**
- **AbyssOS Portal**: https://demiurge.cloud (Full-screen desktop environment)
- **RPC Endpoint**: https://rpc.demiurge.cloud/rpc (JSON-RPC API)

**Current State**: See [Current State Overview](CURRENT_STATE.md) for production deployment status and recent milestones.

## Applications

- [AbyssOS Portal](apps/ABYSSOS_PORTAL.md) - Full-screen desktop environment for Demiurge Blockchain
  - Live at: https://demiurge.cloud
  - Features: Boot screen, AbyssID auth, desktop UI, Chain Ops, window management

## Overview

- [Current State](CURRENT_STATE.md) - Production status and recent milestones
- [Architecture Overview](overview/ARCHITECTURE_DEMIURGE_CURRENT.md)
- [Runtime Modules](overview/RUNTIME.md)
- [Consensus](overview/CONSENSUS.md) *(future)*
- [Tokens: CGT & D-GEN](overview/TOKENS_CGT_DGEN.md) *(future)*
- [State Capsule](overview/DEMIURGE_STATE_CAPSULE.md)
- [Schematic Post Phase 2](overview/SCHEMATIC_POST_PHASE2.md)

## API & Protocol

- [JSON-RPC API](api/RPC.md)
- [Work Claim API](api/WORK_CLAIM.md) *(future)*

## Network

- [Devnet Setup](network/DEVNET.md)
- [Quick Start Guide](network/QUICK_START.md)
- [Quick Start: Fracture](network/QUICK_START_FRACTURE.md)
- [Quick Clone Instructions](network/QUICK_CLONE_INSTRUCTIONS.md)
- Testnet (coming soon)
- Mainnet (coming soon)

## Deployment

- [Node0 / RC0 Deployment Guide](deployment/README_NODE0.md) - **Current production deployment**
- [RC0 Deployment](deployment/DEPLOYMENT_RC0.md)
- [AbyssOS Deployment](apps/ABYSSOS_PORTAL.md#deployment) - HTTPS deployment at demiurge.cloud
- [Complete Deployment Instructions](deployment/COMPLETE_DEPLOYMENT_INSTRUCTIONS.md)
- [Deployment Guide](deployment/DEPLOYMENT_GUIDE.md)
- [OVHCloud Deployment Guide](deployment/OVHCLOUD_DEPLOYMENT_GUIDE.md)
- [Server Setup Overview](deployment/SERVER_SETUP_OVERVIEW.md)
- [Server Setup Next Steps](deployment/SERVER_SETUP_NEXT_STEPS.md)
- [Quick Server Setup](deployment/QUICK_SERVER_SETUP.md)
- [SSH Setup for OVHCloud](deployment/SSH_SETUP_OVHCLOUD.md)
- [How to Add SSH Key](deployment/HOW_TO_ADD_SSH_KEY.md)
- [Add SSH Key to OVHCloud](deployment/ADD_SSH_KEY_TO_OVHCLOUD.md)

## Runtime Modules

- [Bank CGT Module](modules/BANK_CGT.md)
- [Work Claim Module](modules/WORK_CLAIM_MODULE.md)
- UrgeID Registry Module (coming soon)
- NFT D-GEN Module (coming soon)
- Abyss Registry Module (coming soon)

## Operations

- [Monitoring & Health Checks](operations/MONITORING.md) *(future)*
- [Snapshots & Backups](operations/SNAPSHOTS_BACKUPS.md) *(future)*
- [Automated Updates Guide](operations/AUTOMATED_UPDATES_GUIDE.md)
- [Chain Node Info](operations/CHAIN_NODE_INFO.md)
- [Testing Guide](operations/TESTING_GUIDE.md)
- [Media Files Setup](operations/MEDIA_FILES_SETUP.md)
- [Reset Instructions](operations/RESET_INSTRUCTIONS.md)

## Design Documents

- [Economy & CGT Design](design/ECONOMY_DESIGN.md) *(future)*
- [DRC-369 Architecture](design/NFT_ARCHITECTURE_DGEN.md) *(future - DRC-369 standard)*
- [Fracture V1 Implementation](design/FRACTURE_V1_IMPLEMENTATION.md)
- [Fracture V1 Implementation Complete](design/FRACTURE_V1_IMPLEMENTATION_COMPLETE.md)
- [Fracture V1 Audit](design/FRACTURE_V1_AUDIT.md)
- [Fracture V1 Media Files](design/FRACTURE_V1_MEDIA_FILES.md)
- [AbyssID Ritual Implementation](design/ABYSSID_RITUAL_IMPLEMENTATION.md)

## Development

- [Development Status](development/DEVELOPMENT_STATUS.md)
- [Current Status & Action Plan](development/CURRENT_STATUS_AND_ACTION_PLAN.md)
- [Project Utility Overview](development/PROJECT_UTILITY_OVERVIEW.md)

**Current Production Status:**
- Node0: Live at 51.210.209.112
- AbyssOS: https://demiurge.cloud (HTTPS enabled)
- RPC: https://rpc.demiurge.cloud/rpc (HTTPS enabled)

### Milestones

- [Milestone 1 Complete](development/MILESTONE1_COMPLETE.md)
- [Milestone 2 Progress](development/MILESTONE2_PROGRESS.md)
- [Milestone 3 Complete](development/MILESTONE3_COMPLETE.md)
- [Milestone 4 Complete](development/MILESTONE4_COMPLETE.md)
- [Milestone 4 Diagnostic Report](development/MILESTONE4_DIAGNOSTIC_REPORT.md)
- [Milestone 5: Architecture Discovery](development/MILESTONE_5_ARCHITECTURE_DISCOVERY.md)
- [Milestone 5: Awakening](development/MILESTONE_5_AWAKENING.md)
- [Milestone 5: Implementation Summary](development/MILESTONE_5_IMPLEMENTATION_SUMMARY.md)
- [Milestone 5: Improvements Summary](development/MILESTONE_5_IMPROVEMENTS_SUMMARY.md)
- [Milestone 5: Routes](development/MILESTONE_5_ROUTES.md)
- [Milestone 5: Testing](development/MILESTONE_5_TESTING.md)
- [Milestone 6: Genesis](development/MILESTONE_6_GENESIS.md)
- [Milestone 7: Convergence](development/MILESTONE_7_CONVERGENCE.md)

### Phases

- [Phase 4: Audio Implementation](development/phase4_audio_implementation.md)
- [Phase 5: Frontend Update](development/phase5_frontend_update.md)
- [Phase 5 Complete](development/PHASE5_COMPLETE.md)

### Testing

- [Testing Phase 3 Chat](development/TESTING_PHASE3_CHAT.md)
- [Test Features](development/TEST_FEATURES.md)
